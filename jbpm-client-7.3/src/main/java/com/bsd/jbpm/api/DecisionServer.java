...
public class DecisionServer {
	...
	public void disposeAndCreateContainer() {
		...
		String containerId = "oms-process-7.3_1.0.0";//container.getContainerId();
	...
	public void listProcesses() {
		...
		List<ProcessDefinition> findProcessesByContainerId = queryClient
				.findProcessesByContainerId("oms-process-7.3_1.0.0", 0, 1000);
	...
	public void startProcess() {
		...
		long id =  processServicesClient.startProcess("oms-process-7.3_1.0.0",
				"OrderManagement.order-validation", variables);
	...
	public void completeTaskInDetails() {
		...
		List<TaskSummary> mytasks = taskClient.findTasksAssignedAsPotentialOwner("krisv", 0, 10);

		for (TaskSummary taskSummary : mytasks) {
			Map<String, Object> inputData = taskClient
					.getTaskInputContentByTaskId("oms-process-7.3_1.0.0",
							taskSummary.getId());
			...
			if(taskSummary.getStatus().equals("Reserved")){
				System.out.println("Starting task id:" + taskSummary.getId());
			...
			TaskInstance taskInstance=taskClient.findTaskById( taskSummary.getId());

			if ("InProgress".equals(taskInstance.getStatus()) || "Reserved".equals(taskInstance.getStatus())) {
				System.out.println("Completing task id:" + taskSummary.getId());
				taskClient.completeTask("oms-process-7.3_1.0.0", taskSummary.getId(),
						USER, outputData);
			}
		}
	}
