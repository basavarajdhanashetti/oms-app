...
<body>
	...
	<div id="wrapper">
		...
		<form:form commandName="quotationForm">
			<form:hidden path="id" />
				...
				<div class="container-fluid">
					<div class="row">
						<div class="col-lg-12">
							...
							<c:if
								test="${taskSummary.name == 'Quotation Approval' or taskSummary.name == 'Quotation Selection'}">
								...
								<div class="panel-body">
									<div class="row">
										<div class="col-lg-6">
											<div class="form-group">
												<label>Vendor Name</label> ${quotationForm.vendor.name }
											</div>
										</div>
										<div class="col-lg-6">
											<div class="form-group">
												<label>Contact Details</label>
												${quotationForm.vendor.contactNo} -
												${quotationForm.vendor.emailId }
											</div>
										</div>
									</div>
															...
															<tbody>
																<c:forEach items="${quotationForm.items}" var="item"
																	...
																	<tr>
																		...
																		<td>${item.quantity}</td>
																		<td><form:input
																				path="items[${row.index}].unitPrice"
																				class="form-control"
																				id="quot_unitPrice-${row.index}"
																				readonly="${readOnlyVals}"
																				onchange="calculateAmount(${row.index})" /></td>
																		<td><form:input
																				path="items[${row.index}].discount"
																				class="form-control" id="quot_discount-${row.index}"
																				readonly="${readOnlyVals}"
																				onchange="calculateAmount(${row.index})" /></td>
																		<td><form:input
																				path="items[${row.index}].salePrice"
																				class="form-control"
																				id="quot_salePrice-${row.index}" readonly="true"
																				onchange="calculateAmount(${row.index})" /></td>
																	</tr>
									...
									<div class="row">
										...
										<div class="col-lg-6">
											<div class="form-group">
												<label>Tax</label>
												<form:input path="tax" class="form-control" id="tax"
													onchange="calculateTax()" readonly="${readOnlyVals}" />
									...
									<div class="row">
										<div class="col-lg-12">
											<c:if test="${taskSummary.name == 'Amend Quotation'}">
												<button type="submit" class="btn btn-primary"
													formaction="${pageContext.request.contextPath}/quotations/update"
...
function calculateAmount(index){
	...
	var quoteAmount = 0;
	var arrayLen = "${fn:length(quotationForm.items)}";
